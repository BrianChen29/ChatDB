...
# Function to preprocess and format date columns before insertion
def preprocess_dates(dataframe):
    for column in dataframe.columns:
        # Try to detect if the column is a date
        ...
        if sample_value:
            ...
            if dtype == "DATE" and detected_format:
                # Convert the column to the MySQL-compatible date format YYYY-MM-DD
...
def map_pandas_dtype_to_mysql(column, dataframe):
    ...
    if sample_value:
        # attempt to detect datetime format using the datetime module
...
def detect_primary_key(dataframe):
    ...
    for column in dataframe.columns:
        ...
        # Check if the column is a good candidate for primary key
    ...
    return None  # If no suitable column is found

# Function to drop the table if it already exists
...
# Function to create a table dynamically based on CSV column types
def create_table_from_csv(connection, table_name, dataframe):
    ...
    # Dynamically create column definitions based on CSV data types
    columns = []
...
# Function to upload dataset to MySQL based on CSV columns and table columns
def upload_dataset_to_mysql(connection, table_name, csv_file_path):
    try:
        # Read CSV file
        dataset = pd.read_csv(csv_file_path)

        # Preprocess date columns to correct the format
        preprocess_dates(dataset)
        
        # Create the table dynamically based on the CSV structure
...
if __name__ == "__main__":
    ...
    if mysql_conn:
        ...
        if mysql_conn_with_db:
            # Dataset Setup (One-Time Execution)
            coffee_shop_table_name = TABLE_NAME
            coffee_shop_csv_file_path = CSV_PATH  # Replace with your actual CSV file path
