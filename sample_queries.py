...
# Helper function to get column types, keys, and example values
def get_column_info_and_examples(connection, table_name):
    """
    Retrieves column names, data types, keys, and example values from a MySQL table.
    """
    column_info = {}
    examples = {}
    all_columns = []
...
def execute_query(connection, query, as_list=False):
    """
    Executes a query on the MySQL database and returns the result as a Pandas DataFrame.
    """
    with connection.cursor() as cursor:
...
def generate_sql_examples(connection, table_name):
    """
    Generates SQL query examples for all constructs: GROUP BY, WHERE, HAVING, ORDER BY, AGGREGATION.
    """
...
# Main function to handle user requests via sys.argv
def main():
    if len(sys.argv) < 4:
